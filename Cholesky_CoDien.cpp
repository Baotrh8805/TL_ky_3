...
public:
    ...
    void Cholesky() {
        if (!this->IsSquare()) {
            throw invalid_argument("Ma tran khong phai la ma tran vuong");
        }
        if (!this->IsSymmetry()) {
            throw invalid_argument("Ma tran khong doi xung");
        }
        if (!this->IsPositiveDefinite()) {
            throw invalid_argument("Ma tran khong xac dinh duong");
        }
        Matrix L(row, col);
        for(int i = 0; i < row; ++i) {
            for(int j = 0; j < col; ++j) {
        ...
        cout << "Ma tran L^T:" << endl << LT << endl;
        cout << "Ket qua L * L^T:" << endl << L * LT << endl;
    }
