...
for (int y = 0; y < n; y++) {
if (x != i && y != j) {
temp[temp_i][(y < j) ? y : (y - 1)] = A[x][y];
}
}
if (x != i) temp_i++;
}
adjoint[j][i] = (pow(-1, i + j) * determinant(temp, n - 1)) / det;
}
}
for (int i = 0; i < n; i++) {
for (int j = 0; j < n; j++) {
...
double k = (9 * a * b * c - 2 * b * b * b - 27 * a * a * d) / (2 * sqrt(fabs(delta * delta *
delta)));
if (delta > 0) {
if (-1 <= k && k <= 1) {
...
cout << setprecision(4) << fixed << lambda[0] << endl << lambda[1] << endl <<
lambda[2] << endl;
